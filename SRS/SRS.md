...
|----|----------|---------|------|----------|----------|
| UC-01 | 周辺機器ホットプラグ | USBドックに周辺機器（PC/外部ディスプレイ/USBデバイス）をホットプラグで接続する | USBドックと周辺機器は未接続 | USBドックに周辺機器を接続する | 接続後3s以内に、PCが充電中の表示になる/外部ディスプレイに拡張デスクトップが表示される/USBデバイスがPCで認
| UC-02 | PC切り替え | 外部ディスプレイ/USBデバイスの接続先PCを切り替える | USBドックと外部ディスプレイ/USBデバイス/PC_1/PC_2が接続済み | USBドック上のスイッチを操作する | 切り替え後3s以内に外部ディスプレイ/USBデバイスの接続先PCがもう一方のPCに切り替わる。切り替え後も両PCへの給電は継続する |
| UC-03 | スリープ給電 | PC未接続時もUSBデバイスへの給電を継続する | USBドックとPCが未接続 | 特になし | USBデバイスへ給電が行われる |
| UC-04 | 過電流保護 | USBの故障を防止する | USB デバイス短絡 | 特になし | 5ms以内に出力遮断／リセット後自動復帰 |
...
| ID | 優先度 | 機能要求 | 根拠 |
|----|--------|------|------------|
| FR-00x | MUST | システムはHDMIを通じて最大2つのFHDディスプレイに画面出力すること | UC-01 |
| FR-00x | SHOULD | システムはHDMIを通じて最大2つの4Kディスプレイに画面出力すること | UC-01 |
| FR-00x | MUST | システムは最大3台のUSB3.2デバイスをアクティブホストに列挙すること | UC-01 |
| FR-00x | MUST | システムはUSB typeCを通じてPCに給電をすること | UC-01 |
| FR-00x | MUST | システムはPC_1に接続された外部ディスプレイ/周辺機器をPC_2に接続するように切り替えること | UC-02 |
| FR-00x | MUST | システムは最大3台のUSB3.2デバイスに給電を行うこと | UC-03 |
| FR-00x | MUST | システムは最大3台のUSB3.2デバイスに過電流が流れた際に5ms以内に給電を中止し、自動復帰すること | UC-04 |
